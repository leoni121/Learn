...
> （1）Event Loop是一个程序结构，用于等待和发送消息和事件。（a programming construct that waits for and dispatches events or messages in a program.）
...
>
> （3）***Event loop是一种程序结构，是实现异步的一种机制。***Event loop可以简单理解为：
>
...
> 4. 主线程不断重复上面的第三步。
>
> （4）**JavaScript（引擎）是单线程的，Event loop并不属于JavaScript本身，但JavaScript的运行环境是多线程／多进程的，运行环境实现了Event loop。**
>
> 1. 所有任务都在主线程上执行，形成一个执行栈（execution context stack）。
> 2. 主线程之外，还存在一个"任务队列"（task queue）。系统把异步任务放到"任务队列"之中，然后主线程继续执行后续的任务。
> 3. 一旦"执行栈"中的所有任务执行完毕，系统就会读取"任务队列"。如果这个时候，异步任务已经结束了等待状态，就会从"任务队列"进入执行栈，恢复执行。
...
> Node很多API都是基于事件订阅完成的，这些API的回调应该都在***poll阶段***完成。
