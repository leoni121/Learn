...
//　即src/compiler/to-function.js 文件中 createCompileToFunctionFn 函数
// 返回的 compileToFunctions 函数。
import {
	compileToFunctions
} from './compiler/index'
import {
	...
	if (!options.render) {
		...
		if (template) {
			...
			const {
				...
				// 作用是对浏览器的怪癖做兼容，并且这两个常量的类型都是布尔值。
				outputSourceRange: process.env.NODE_ENV !== 'production',
				shouldDecodeNewlines,
				shouldDecodeNewlinesForHref,
				// 下面这两个选项只有在创建完整版 Vue 的时候才会用到，
				// 大家不要忘了 entry-runtime-with-compiler.js 
				// 这个文件是完整版 Vue 的入口，也就是说运行时版的 Vue 
				// 压根不存在这些内容所以自然不会起作用。
