...
5. **X-XSS-Protection** 
   ...
   对于反射型XSS进行一些防御，在现代浏览器中基本上是不必要的，但是当网站实施一个强大的[`Content-Security-Policy`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Content-Security-Policy)来禁用内联的JavaScript (`'unsafe-inline'`)时, 他们仍然可以为尚不支持 [CSP](https://developer.mozilla.org/en-US/docs/Glossary/CSP) 的旧版浏览器的用户提供保护
