...
HTTP协议是Hyper Text Transfer Protocol（超文本传输协议）的缩写,是基于客户端/服务端（C/S）的架构模型，是一个**无状态、无连接、媒体独立的请求/响应协议**。用于从**万维网（WWW:World Wide Web ）服务器传输超文本到本地浏览器**的传送协议
